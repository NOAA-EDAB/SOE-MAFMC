...
#       caption = "Mid-Atlantic Bight feeding guilds listed with key species.",
#       align = 'c',
#       booktabs = T) %>% 
#   #kable_styling(latex_options = "striped") %>%
...
#Patchwork plot layout
rec_effort + rec_div + rec_anglers +
  plot_layout(ncol =  1) &
  theme(plot.margin = unit(c(0, 0, 0, 0), "cm"))
...
```{r comm-reliance-maps, fig.width = 8, fig.height = 4.5}
# Set lat/lon window for maps
xmin = -78
xmax = -70
ymin = 35
...
eng_rel <- ecodata::eng_rel %>%
  ...
  dplyr::rename(comeng = ComEng_NE16_ct,
                ...
                State = STATEABBR) %>% 
  mutate(receng = as.factor(receng))
...
```{r comm-reliance-maps-outlined, fig.width = 8, fig.height = 4.5}
com <- eng_rel %>% 
  dplyr::select(comeng, comrel, Latitude, Longitude) %>% 
  filter(comeng != 0,
         comrel != 0) %>% 
...
int_join <- data.frame(STATEABBR = rep(unique(ecodata::eng_rel[ecodata::eng_rel$REGION == region,]$STATEABBR), each = 4),
                       Var = factor(1:4))
...
Patterns for groups of species that feed on similar prey can indicate how overall ecosystem conditions are changing and provide context for individual species stock assessments. The information presented in Figure \ref{nefsc-biomass} is taken from NEEFSC bottom trawl surveys during the spring and fall. We also include data from the NorthEast Area Monitoring and Assessment Program (NEAMAP), which is an inshore sampling initiative surveying the region between Cape Cod, MA and Cape Hatteras, NC.

## Trends in biomass (`r epu_abbr`)

```{r nefsc-biomass, fig.cap = paste0("Fall (left) and spring (right) surveyed biomass in the Mid-Atlantic Bight. Data from the NEFSC Bottom Trawl Survey are shown in black, with NEAMAP shown in red. \\label{nefsc-biomass}"), fig.width=8, fig.asp = 0.75}
...
ggplot(data = total_surv, aes(x = Time, y = Value, color = Var)) +
  ...
  #Highlight last ten years
  annotate("rect", fill = shade.fill, alpha = shade.alpha,
      xmin = x.shade.min + 1 , xmax = x.shade.max + 1,
      ymin = -Inf, ymax = Inf) +
...
```{r MAB-wind, eval=F, fig.width = 8, fig.asp=0.35, fig.cap = "Total wind speed, storminess, and turbulent kinetic energy during the summer in the Mid-Atlantic Bight."}
...
tws <- summer_winds %>%
  ...
  mutate(Var = str_to_title(word(Var, -1))) %>% 
  ggplot()+ #plot
...
tws + helicity + tke + plot_annotation(title = "Summer Winds") &
  theme(plot.margin = unit(c(0, 0.1, 0, 0), "cm")) 

```


```{r ocean-temp, fig.width = 6, fig.asp = .35, fig.cap = paste0("Ocean temperature trends in the ",region,". \\label{ocean-temp}"), warning=F}

temp <- oceantemp_insitu %>% 
  filter(EPU == epu_abbr) %>% 
...
```{r ocean-sal, fig.width = 6, fig.asp = .35, fig.cap = paste0("Ocean salinity trends in the ",region,". \\label{ocean-sal}"), warning=F}
