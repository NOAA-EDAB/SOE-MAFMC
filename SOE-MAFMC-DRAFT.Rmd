...
```{r nefsc-biomass, fig.cap = paste0("Fall (left) and spring (right) NEFSC survey biomass in the ",region,". \\label{nefsc-biomass}")}
...
(chl_oc_cci <- pp %>%
  filter(str_detect(Var, "MONTHLY_CHLOR_A_MEDIAN OC-CCI PAN")) %>% 
...
(pp_seawifs <- pp %>%
  filter(str_detect(Var, "MONTHLY_PPD_MEDIAN OC-CCI BERENFELD")) %>% 
  separate(.,Time, into = c("Year","Month"), sep = 4) %>% 
  ggplot() +
    geom_point(aes(x = Year, y = Value)) +
    facet_wrap(Month~., ncol = 12) +
